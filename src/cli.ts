...
import { ensurePrivateDirectory, getIpcToken, getPairingToken, getRuntimePaths } from "./config.js";
import {
...
import { discoverLocalChromiumExtensionOrigins } from "./chromium-extension-discovery.js";
import { configureZenFlatpakNativeMessaging } from "./zen-flatpak.js";
import { purgeOwnedApplicationDirectories } from "./purge-data.js";
import { browserLaunchEnvironment } from "./browser-environment.js";
...
async function handOffSetupToPersistentInstall(setupArgs: string[]): Promise<boolean> {
  ...
  try {
    ...
    if (await realpath(installedCli) === await realpath(fileURLToPath(import.meta.url))) return false;
  } catch {
