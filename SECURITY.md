...
2. Remove the exact native registration and stop the per-user daemon with `npx browseweave@0.1.0-beta.6 local-uninstall`, or use the platform's service manager when the CLI is unavailable.
3. Run `npx browseweave@0.1.0-beta.6 doctor` and preserve only the metadata audit log.
