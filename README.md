...
- Serve more than one MCP client session from a single browser profile without letting them close or drive each other's tabs.
...
More than one MCP client session can use the same browser at the same time. Each
session gets an identity from its own MCP server process, and every tab
BrowseWeave opens records the session that opened it.

- `browser_close_tab` and `browser_cleanup_tabs` act only on the caller's tabs. A
  `tab_ids` list is intersected with them, so ending one session never closes
  tabs another session is still reading.
- Clicking, typing, navigating, or otherwise driving a tab another agent opened
  is refused with `tab_owned_by_another_agent`. Reading it is allowed, because
  reading changes nothing.
- `browser_list_tabs` reports `managed_by_you` next to `managed`.
- Each agent may keep 10 managed tabs open, and one browser profile holds at most
  20 across every connected agent.

Tabs you opened yourself are not managed by BrowseWeave and stay available to
every agent, exactly as before. The extension popup's own cleanup button closes
every managed tab regardless of owner: you own the browser, so agent scoping does
not apply to you.

Separate browser profiles remain the stronger split, since they also separate
credentials, approvals, and browsing state. Use `setup --new-profile` for that.
...
That short form covers only the routine transition categories `form_submit` and `external_navigation`. Ordinary reads, typing, scrolling, same-site navigation, and safe searches already run without a prompt.

Every other category — `message`, `visual_click`, `delete`, `payment`, `security`, `password`, `2fa`, and `file_attach` — discloses data, is hard to reverse, affects account security, or relies on a lower-confidence visual target, so the short form never sweeps it in. Name each one you want to run unattended:
...
```json
{
  "autonomous_actions": {
    "enabled": true,
    "categories": [
      "form_submit", "external_navigation", "message", "delete",
      "security", "visual_click", "payment", "password", "2fa", "file_attach"
    ]
  }
}
```

An explicit list replaces the default entirely, so it can also be narrower than the short form:

```json
{ "autonomous_actions": { "enabled": true, "categories": ["external_navigation"] } }
```

Naming `file_attach` widens only who may confirm an attachment, never which files are readable: the `file_attach` allowlist still bounds the exact directories, and a path outside them is refused before any approval is considered.
...
`npx browseweave@0.1.0-beta.16 doctor` reports the policy file path and the categories the running daemon actually loaded. The service also logs the enabled categories at startup. Enabling this is a deliberate trade: every category you list runs unattended, so the agent acts on that class of target without asking first. List only the categories you are willing to have the agent take on its own.
...
`browser_query` answers what a snapshot cannot. A snapshot reports what a reader can act on, so it carries almost nothing from `<head>` and no counts at all: asking it for meta tags, JSON-LD, `hreflang`, or alt-text coverage is not slow, it is impossible. Give it a CSS selector and the attributes to project, and it returns one bounded row per match. Put the predicate in the selector — `img:not([alt])` counts missing alt text, `a[rel~=nofollow]` counts nofollow links — and `matched` reports the full count even when `limit` returns fewer rows, so `limit: 0` counts without reading. Matching happens across open shadow roots, and each row carries a `ref` usable with `browser_click` and `browser_type`.

The selector is matched, never evaluated, so this works on pages whose Content-Security-Policy forbids script execution. Values are not returned raw: URL attributes are resolved and redacted, and a form value passes through the same field classification a snapshot uses, so querying `input` cannot lift a password, one-time code, or card number into model context. It is read-only and never consumes an approval.
...
```bash
npm install --global browseweave@0.1.0-beta.16
...
After a global install, any pinned `npx browseweave@0.1.0-beta.16 ...` example
...
```bash
npx browseweave@0.1.0-beta.16 setup --browser chrome --client codex
```

Other explicit combinations are:

```bash
npx browseweave@0.1.0-beta.16 setup --browser zen --client claude-code --client cursor
npx browseweave@0.1.0-beta.16 setup --browser chrome --browser-path /absolute/path/to/chrome --client codex
npx browseweave@0.1.0-beta.16 setup --browser chrome --new-profile --client codex
npx browseweave@0.1.0-beta.16 setup --browser chrome --client opencode --opencode-v2
```
...
```bash
npx browseweave@0.1.0-beta.16 mcp-config generic
```
...
```bash
npx browseweave@0.1.0-beta.16 mcp-add opencode --opencode-v1
npx browseweave@0.1.0-beta.16 mcp-add opencode --opencode-v2
```
...
```bash
npx browseweave@0.1.0-beta.16 local-install
```
...
```bash
npx browseweave@0.1.0-beta.16 doctor
npx browseweave@0.1.0-beta.16 extension-path chrome
npx browseweave@0.1.0-beta.16 extension-path zen
```
...
```bash
npx browseweave@0.1.0-beta.16 local-uninstall
```
