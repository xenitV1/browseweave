...
- Use `browser_query` when the answer is page data a snapshot does not carry: `<head>` metadata, JSON-LD, `hreflang`, link or image coverage. Give a CSS selector and the attributes to project; put the predicate in the selector (`img:not([alt])`, `a[rel~=nofollow]`) and pass `limit: 0` to count without reading, since `matched` always reports the full count. Never reach for script execution to gather page data.
